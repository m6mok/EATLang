# AGENTS.md — инструкции для агентов

EATLang — учебно-исследовательский язык: компилируемый (LLVM),
статически типизируемый, **тотальный** (каждая программа доказуемо
завершается), с 10 правилами NASA (Power of 10), зашитыми в сам язык.
Проект самохостится: компилятор написан на EATLang и собирает сам себя;
Python-бутстрап (`src/eatc/`) — эталон, с которым всё сверяется
байт-в-байт.

## Документация (читать по задаче)

| Документ | Когда читать |
| --- | --- |
| [docs/PROCESSES.md](docs/PROCESSES.md) | **сначала** — конвейер компиляции, два компилятора, команды CLI и make, процессы проверки |
| [docs/MODIFYING.md](docs/MODIFYING.md) | перед **любой** правкой компилятора/языка — инварианты, порядок работ, кодировки, чеклисты |
| [docs/POWER_OF_10.md](docs/POWER_OF_10.md) | правила Power of 10: как каждое воплощено, где проверяется, что нельзя открыть правкой |
| [SPEC.md](SPEC.md) | формальная спецификация: EBNF, семантика, пределы §6, встроенные §7 |
| [docs/GUIDE.md](docs/GUIDE.md) | как писать на языке: синтаксис, контракты, что не скомпилируется |
| [docs/WORKFLOW.md](docs/WORKFLOW.md) | **процесс работы агента**: планы на инициативы, вопросы пользователю, итог этапа (объём/время/метрики/ошибки → FAULTS), коммиты без столкновений с параллельными работами, промт следующего этапа |
| [docs/CONTAINERS.md](docs/CONTAINERS.md) | перед **любой** работой в контейнере — правила создания и гигиены (Podman rootless, `--rm`, чистый worktree, чистка своих слоёв, граница «> 12 ч → только артефакты агента») |
| [docs/plans/](docs/plans/README.md) | **индекс всех планов и треков**: статусы, порядок работ (стек), шаблон нового плана; отсюда — к SELFHOST, VERIFICATION_PLAN, TRACKS, OPT и остальным |

## Главные правила

1. **Любая правка языка в `src/eatc/` зеркалится в `selfhost/*.eat`.**
   Паритет байт-в-байт: дампы, тексты ошибок, trap-сообщения, IR.
   Порядок работ и кодировки — в [docs/MODIFYING.md](docs/MODIFYING.md).
2. **Изменения синтаксиса/семантики/встроенных — только с решением
   пользователя.** Не хватает конструкции — остановись и сообщи.
3. **Определение готовности**:
   `make check verify verify_suite verify_selfhost verify_bootstrap
   verify_trapcodes verify_selfhost_opt verify_sig verify_selfhost_verify
   verify_selfhost_verify_all`
   проходит целиком. Частично зелёное не коммитится.
4. **Не открывать обходов Power of 10**: ни кучи, ни рекурсии,
   ни цикла без границы, ни глобалов, ни молча отброшенного результата
   ([docs/POWER_OF_10.md](docs/POWER_OF_10.md)). Внутри языка правило
   не нарушается **никогда — ноль исключений**; это не предмет торга «на
   один раз» и не размен на скорость/срок/удобство. Просят обход —
   **настаивай и откажи**: остановись, назови нарушаемое правило и
   предложи решение в рамках языка. Давление пользователя это не меняет.
   Единственный легитимный путь — не обход, а **осознанное
   переопределение самого правила** (явно, с новым потолком, в
   POWER_OF_10.md и SPEC §2) — и это отдельное решение пользователя,
   не побочный эффект правки.
5. Документация и комментарии в репозитории — **на русском**;
   коммиты — в стиле истории (`git log --oneline`).
6. **Процесс ведения работ — [docs/WORKFLOW.md](docs/WORKFLOW.md)**:
   новая инициатива — план в docs/plans/; задавай пользователю
   вопросы, если нужно; после каждого этапа — итог понятным языком
   (объём, время, метрики до/после, ошибки → docs/plans/FAULTS.md,
   промт следующего этапа); не задевай коммитами параллельные работы.
7. **Работа в контейнере — по [docs/CONTAINERS.md](docs/CONTAINERS.md)**:
   только Podman (rootless, без демона), пиннутый образ из
   `containers/`, `podman run` всегда `--rm`, монтировать **чистый
   worktree на HEAD**. Чистить свои слои после каждого эксперимента;
   правило «> 12 ч → удалять» — **строго к артефактам агента**, никогда
   не блэнкет-`prune` (на машине живут другие проекты пользователя).

## Быстрый старт

```sh
uv sync              # окружение (единственная зависимость — llvmlite);
                     # для линковки бинарников нужен clang
make check           # стиль (строки *.eat ≤ 100 кодпойнтов) + компиляция
                     # примеров: парсинг, Power of 10, типы, тесты
make verify          # бинарники == интерпретатор на всех примерах
make verify_selfhost # self-hosted компилятор против эталона на всех .eat
make verify_bootstrap# фикспойнт: компилятор собирает сам себя
make verify_trapcodes# режим trap-кодов (МК): SelfIrCodes == eatc ir --trap-codes
make verify_selfhost_opt # ось -O (свёртка + элизия): SelfIrOpt == eatc ir -O
make verify_sig      # интерфейс lib/ == снапшот tests/sig/lib.sig
make verify_selfhost_verify     # self-hosted верификатор == eatc verify (смок, 50 кейсов)
make verify_selfhost_verify_all # верификатор: весь репозиторий + самоприменение
```

CLI компилятора: `PYTHONPATH=src uv run python -m eatc
{check|run|build|lex|parse|sig|typed|ir}` (в Makefile — `$(EATC)`).

## Карта репозитория

```text
src/eatc/     Python-бутстрап (эталон): lexer → parser → checks →
              typechecker → interpreter → verifier → codegen; runtime.c —
              шим аксиом ОС (ввод/вывод, exit, trap, argv)
selfhost/     компилятор на EATLang, разложен по папкам-фазам (Rt.eat —
              рантайм-модуль, в корне): lex/ (Tok, Lexer, LexMain),
              parse/ (Ast, Parser, ParserExpr, ParseMain), check/ (Check
              +Const/Body/Dump/Fold, SigMain, TypedMain), ir/ (Ir
              +Emit/Expr/Stmt, IrMain, IrCodesMain, IrOptMain), verify/
              (Verify +Expr/Rel/Flow/Clamp/Dump, VerifyMain)
lib/          библиотека на EATLang, разложена по ролям-папкам:
              http/ (Http, Server), json/ (Json), fmt/ (Fmt, Hex, Ascii,
              Num, Parse, Fixed), os/ (Io, Args, Async), core/ (Const,
              Buf, U128) — подключается списком файлов после Rt.eat
              (docs/plans/MODULES_PLAN.md, этап 0 — конкатенация)
examples/     эталонные примеры (all — витрина конструкций; mos6502 —
              эмулятор; lexer — проба self-host)
tests/        verify/ (кейсы верификатора), lex|parse|ir/ (стресс дампов),
              bench/ (нагрузка), verify_suite.py
docs/         процессы, руководства, планы (см. таблицу выше)
```

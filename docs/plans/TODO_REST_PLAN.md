...
bounded-пул задач в кадре `let` в `main`, переживающий соединения через
...
- **Состояние без кучи и глобалов.** Коллекция задач — фиксированный
  массив `[Todo; TODO_CAP]` в структуре `Store`, живущей как `let store`
  в кадре `main`. Мутация — только через `let self`-методы `Store`
  (правило: единственный писатель в чужую память — `let self`,
...
struct Store  { items: [Todo; 256], next_id: u32, count: u32
                # let self: create/update/remove/toggle
                # self:     find(id)->idx|NONE, serialize_list(JOut),
                #           serialize_one(JOut, idx)
}
```

- **Store в кадре `main`.** `let store: Store = store_new()` до `loop`;
  диспетч соединения вызывает методы store. Мутация — `let self`.
- **Маршруты (метод + путь):**
...
состояние — `let` в кадре `main` (нет мутабельных глобалов), результаты

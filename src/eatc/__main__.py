...
python -m eatc verify <файл>      — эталонный дамп верификатора:
                                    ...
                                    (сверка с self-hosted верификатором,
                                    SELFHOST_VERIFIER_PLAN); `-O` —
                                    решения под конвейером оси
                                    (fold перед verify)
...
def cmd_verify(path: str, opt: bool = False) -> int:
    """Дамп верификатора (SELFHOST_VERIFIER_PLAN этап 0): обязательства
    в стабильном порядке (строка → колонка → вид) + футер-агрегат —
    эталон сверки с self-hosted верификатором. `-O` — дамп решений под
    конвейером оси (fold перед verify, порядок как в build): сверка
    множества решений прохода элизии ДО сверки эмиссии (этап 5,
    урок SELFHOST_OPT_PLAN §9)."""
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) == 2 and argv[0] == "verify":
        return cmd_verify(argv[1], opt=opt)
    ...
    print(
        ...
        "lex <файл> | "
        "parse <файл> | verify <файл> [-O] | "
        "ir <файл> [--trap-codes] [-O] | "
        "stream <файл>) "
        "[--lib DIR]...",
        file=sys.stderr,
    )
